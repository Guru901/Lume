...
impl<S: Schema + Debug> Row<S> {
    ...
    #[cfg(feature = "postgres")]
    /// Converts POSTGRES rows to type-safe Lume rows.
    ///
    /// This method extracts data from POSTGRES rows and converts them to
    /// type-safe `Row<S>` instances. It uses the schema's column metadata
    /// to determine the correct types for extraction.
    ///
    /// # Arguments
    ///
    /// - `rows`: A vector of POSGTRES rows from a database query
    ///
    /// # Returns
    ///
    /// A vector of type-safe `Row<S>` instances
    ...
    ///
    /// The method uses the schema's column metadata to determine how to
    /// extract values from POSTGRES rows:
    ///
    /// - `VARCHAR`, `TEXT`    → `String`
    /// - `INTEGER`, `INT4`    → `i32`
    /// - `BIGINT`, `INT8`     → `i64`
