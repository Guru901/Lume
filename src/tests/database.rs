...
mod tests {
    ...
    fn test_generate_migration_sql_mysql() {
        ...
        assert!(sql.contains("__title VARCHAR(255) NOT NULL"));

        assert!(sql.contains("CREATE INDEX _idx_Users__username ON Users (_username);"));
        assert!(sql.contains("CREATE INDEX _idx_Posts___title ON Posts (__title);"));
    }
    ...
    fn test_generate_migration_sql_postgres() {
        ...
        assert!(sql.contains("_id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS _idENTITY"));
        assert!(sql.contains("_username VARCHAR(255) NOT NULL"));
        assert!(sql.contains("__title VARCHAR(255) NOT NULL"));
